        ...
        const rookies = useMemo(() => {
            return Object.entries(playersData)
                .filter(([pid, p]) => {
                    ...
                    // Must have DHQ value OR be on an NFL team OR be an active IDP prospect
            ...
            <div style={{ padding: '16px' }}>
                ...
                {activeView === 'board' && (() => {
                    ...
                    <div>
                        ...
                        {/* Expanded player card — ABOVE the boards for visibility */}
                        {expandedDraftPid && (() => {
                                    ...
                                    <div style={{ flex: 1 }}>
                                      ...
                                      <div style={{ display: 'flex', gap: '4px', marginTop: '8px' }}>
                                        {Object.entries(tagDefs).map(([tKey, tDef]) => (
                                          <button key={tKey} onClick={e => { e.stopPropagation(); setBoardTags(prev => ({ ...prev, [r.pid]: prev[r.pid] === tKey ? undefined : tKey })); }} title={tDef.label}
                                            style={{ fontSize: '0.7rem', padding: '2px 8px', border: '1px solid ' + (boardTags[r.pid] === tKey ? tDef.color+'55' : 'rgba(255,255,255,0.1)'), borderRadius: '4px', cursor: 'pointer', background: boardTags[r.pid] === tKey ? tDef.color+'20' : 'rgba(255,255,255,0.03)', color: boardTags[r.pid] === tKey ? tDef.color : 'var(--silver)', fontFamily: 'Oswald' }}>{tDef.icon} {tDef.label}</button>
                                        ))}
                                      </div>
                                    </div>
                                  </div>
                        ...
                        {/* Old expanded card location removed — now renders above the boards */}
